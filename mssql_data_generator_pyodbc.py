...
def ensure_database(server, user, password, database, port):
    ...
    # pyodbc requires a formatted connection string. 
    # TrustServerCertificate=yes is required for local Docker environments.
    conn_str = f"DRIVER={{ODBC Driver 18 for SQL Server}};SERVER={server},{port};DATABASE=master;UID={user};PWD={password};TrustServerCertificate=yes;"
...
def run(server, user, password, database, port=1433, total_records=1000, batch_size=100):
    ...
    # Main connection to the target database
    conn_str = f"DRIVER={{ODBC Driver 18 for SQL Server}};SERVER={server},{port};DATABASE={database};UID={user};PWD={password};TrustServerCertificate=yes;"
